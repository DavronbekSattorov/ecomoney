...
<div
    ...
    class="post-container     rounded-lg flex  mt-4 bg-gray-100 dark:bg-gray-800  "
>

    <div class="flex  md:flex-row flex-1  pt-1 text-gray-600 dark:text-gray-300 text-xs ">
        <div class="w-full flex flex-col justify-between mx-4 ">
            <div class="flex flex-row items-center justify-between  dark:bg-gray-800 bg-gray-100  z-10">
                <div class="flex flex-row items-center ">
                    <a href="{{route('user.show', $post->user->username)}}" class="flex flex-row items-center md:hover:underline underline-offset-2 group text-base">
                        <img src="{{ $post->user->getProfilePhotoUrlAttribute() }}"
...
{{--                                                $dispatch('custom-show-mark-post-as-spam-modal')--}}
                                ...
                                @can('update', $post)
                                    <li>
                                        ...
                                        "
                                            class=" block transition duration-150 ease-in px-3 py-3 flex justify-between items-center group md:hover:bg-gray-200/75 md:dark:hover:bg-gray-600 md:active:bg-gray-200 md:dark:active:bg-gray-500"
                                        >
                                            <span>
                                                Edit Post
                                            </span>
            ...
            <h1 class="text-center text-gray-600 dark:text-white  mt-2   bg-gray-100 dark:bg-gray-800   rounded-t-lg pl-1">
                <a  class="post-link  tracking-wider font-medium sm:text-4xl text-2xl
                    "
                >
            ...
            <div
                ...
                class="bg-gray-100 dark:bg-gray-800 dark:text-white mt-8"
            >
                <div
...
{{--                        }"--}}
                    class="text-gray-600 dark:text-white text-sm  sm:text-xl ck-content ck-content-view font-light
                        pl-1 tracking-wider break-words" >
                    @admin
                    @if ($post->spamPosts()->count() > 0)
                        <div class="text-red-500 mb-2">Spam Reports: {{ $post->spamPosts()->count() }}</div>
                    @endif
                    @endadmin
                    {!! $post->description !!}
                </div>
            ...
            <div class="cursor-pointer ">
                <a href="https://www.google.pl/maps/place/{{$post->location}}">
                    <img src="{{asset('/images/map.jpg')}}" class="h-32 w-full object-cover" alt="">
                </a>
            </div>
            @foreach($post->wasteTypes as $wasteType)
                <div class="flex justify-start items-center text-sm mt-6">
                    <span>
                        Waste type
                    </span>
{{--                    <a href="{{route('waste_type.show', $wasteType->slug)}}" class="flex flex-row items-center md:hover:underline underline-offset-2 group ml-2">--}}
...
{{--                    </a>--}}
                    @foreach($post->wasteTypes as $wasteType)
                        <a href="{{route('waste_type.show', $wasteType->slug)}}" class=" flex flex-row items-center md:hover:underline underline-offset-2 group bg-gray-200 dark:bg-gray-600 rounded-full px-3 py-2 ml-2">
                            {{--                <img src="{{ $wasteType->getProfilePhotoUrlAttribute() }}"--}}
                    ...
                    @endforeach
                    <a disabled  class=" flex flex-row items-center  underline-offset-2 group bg-gray-200 rounded-full dark:bg-gray-600 rounded-full px-3 py-2 ml-2 ">
                        {{--                <img src="{{ $wasteType->getProfilePhotoUrlAttribute() }}"--}}
                        {{--                     alt="avatar" class="w-6 h-6 object-cover rounded-full md:group-hover:ring-2 ring-main-green-light">--}}
                        <div class=" truncate ">
                            {{$post->option}}
            ...
            <div wire:ignore class="w-1/4 flex items-center" >
                <a href="{{    route('post.show', $post)     }}#post-comments" class="group p-2 font-normal md:hover:text-gray-800 rounded-lg transition-transform md:hover:transition-all ease-out duration-100 md:hover:bg-gray-200/75 md:dark:hover:bg-gray-600 md:active:bg-gray-200 md:dark:active:bg-gray-500 flex items-center md:dark:hover:text-white" >
                    ...
                    <div class="ml-1  ">
                        {{ $post->comments()->count() }} <span class="hidden sm:inline"> Comments</span>
                    </div>
                </a>
            </div>
            <div  class="w-1/4 relative "
                ...
                >
                    ...
                    <a
                        ...
                        target="_blank"
                        href="https://t.me/share/url?url={{route('post.show', $post)}}"
                    >
                        <svg
                    ...
                    <a
                        ...
                        target="_blank"
                        href="https://wa.me/?text={{route('post.show', $post)}}"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="24"
                    ...
                    <a
                        ...
                        target="_blank"
                        href="https://twitter.com/intent/tweet?text={{ $post->title }}&amp;url={{route('post.show', $post)}}"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="24"
